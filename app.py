...
def get_gspread_client():
    """
    Ek hi gspread client banayein aur cache karein taake har save par
    naya connection na banana pade (pehle code me har save call par
    naya client + fresh credentials load ho raha tha).
    """
    creds_dict = dict(st.secrets["gcp_service_account"])
    # Secrets me private_key ke andar "\\n" aa jata hai jise real newline
    # me convert karna zaroori hai, warna gspread auth fail hoti hai.
...
def save_sheet_data(worksheet_name, df):
    """Google Sheet me data save/overwrite karne ke liye helper function"""
    ...
    # NaN values gspread.update() me error deti hain (JSON serializable nahi),
    # isliye unhe empty string se replace karein.
    df_clean = df.fillna("")
    worksheet.clear()
...
def clean_products_df(df):
    """Google Sheet se aane wala data hamesha string hota hai,
    isliye numeric columns ko sahi type me convert karna zaroori hai."""
    df = df.copy()
